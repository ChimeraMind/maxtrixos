...
release_common.check_leaking_buckets() {
    ...
    local private_git_repo_path="${imagedir}/${DEFAULT_PRIVATE_GIT_REPO_PATH}"
    if [ -d "${private_git_repo_path}" ] && [ "$(ls -A "${private_git_repo_path}")" ]; then
        echo "ERROR: Leaking files found in private git repo path: ${DEFAULT_PRIVATE_GIT_REPO_PATH}" >&2
